...
class ChessGUI:
    def __init__(self, root):
        ...
        # Chessboard Canvas (with padding for coordinates)
        canvas_width = BOARD_SIZE * SQUARE_SIZE + 32
        canvas_height = BOARD_SIZE * SQUARE_SIZE + 32
        ...
        # --- Best Move for White button ---
        self.best_white_button = tk.Button(frame, text="Best Move for White", command=self.best_move_white)
        self.best_white_button.pack(pady=(0, 6))
    ...
    def draw_board(self):
        ...
        offset = 16  # Padding for coordinates

        # Draw squares
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                x1 = col * SQUARE_SIZE + offset
                y1 = row * SQUARE_SIZE + offset
                x2 = x1 + SQUARE_SIZE
                y2 = y1 + SQUARE_SIZE
        ...
        for col in range(BOARD_SIZE):
            x = col * SQUARE_SIZE + SQUARE_SIZE // 2 + offset
            y = BOARD_SIZE * SQUARE_SIZE + offset + 10
            file_char = chr(ord('a') + col)
            self.canvas.create_text(x, y, text=file_char, font=("Arial", 14, "bold"))

        # Draw rank labels (1-8) at left
        for row in range(BOARD_SIZE):
            x = offset - 10
            y = row * SQUARE_SIZE + SQUARE_SIZE // 2 + offset
            rank_char = str(8 - row)
            self.canvas.create_text(x, y, text=rank_char, font=("Arial", 14, "bold"))
    ...
    def on_click(self, event):
        ...
        row = 7 - ((event.y - offset) // SQUARE_SIZE)
